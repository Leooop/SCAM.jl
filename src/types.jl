...
# RHEOLOGY type :
const N = Number
@kwdef struct Rheology{t1<:N,t2<:N,t3<:N,t4<:N,t5<:N,t6<:N,t7<:N,t8<:N}
    μ::t1 = 0.6 # Friction coef
    β::t2 = 0.1 # Correction factor
    K₁c::t3 = 1.74e6 # Critical stress intensity factor (Pa.m^(1/2))
    a::t4 = 1e-3 # Initial flaw size (m)
    ψ::t5 = atand(0.6)# crack angle to the principal stress (degree)
    D₀::t6 = 0.1# Initial flaw density
    n::t7 = 34.0 # Stress corrosion index
    l̇₀::t8 = 0.24 # Ref. crack growth rate (m/s)
end
...
(σy::CoulombYieldStress)(p) = (ϕ = atan(σy.μ);
p * sin(ϕ) + σy.C * cos(ϕ))
...
(σy::DamagedViscoPlasticYieldStress)(p) = (ϕ = atan(σy.μ);
p * sin(ϕ) + σy.C * cos(ϕ))
